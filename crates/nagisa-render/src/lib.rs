...
//! 楷 / 等宽)、链接(图片点不了,取文字按
//! 强调色渲染)、行内与块级代码(语言标签渲染在盒角)、有序 / 无序列表(可嵌套)、任务列表
//! (`- [ ]` / `- [x]` → `□` / `✓`)、引用、分割线、图片(缩放 / 对齐 / 图注)、左中右两端
//! 对齐、多栏并排([`Columns`])、表格([`Table`]:自适应列宽 / 限宽 / 铺满可用宽
//! (`expand`)/ 按列行格上色 / 紧凑度与网格可调)。标记语言对应是 Markdown 基底加少量扩展(`==高亮==`、`[文字]{属性}`、
//! `::: 围栏`、GFM 表格),见 [`parse_markup`];构建器见 [`Doc`]。
//!
//! # 输出与配置
//!
//! 都在 [`RenderOptions`]:
