...
pub enum Request {
    ...
    Friend {
        initiator: Uin,
        initiator_uid: Option<String>,
        comment: String,
        via: String,
        source_group: Option<Uin>,
        target_user_id: Option<Uin>,
        state: RequestState,
        time: Option<i64>,
        is_filtered: bool,
        token: RequestToken,
    ...
    pub fn peer(&self) -> Option<Peer> {
        ...
        match self {
            ...
            Event::Notice(n) => match n {
                ...
                N::FriendNudge { user, .. } | N::FriendFileUpload { user, .. } | N::FriendAdd { user } => {
                    Some(Peer::friend(*user))
                }
            ...
            Event::Request(r) => match r {
                ...
                R::GroupJoin { group, .. } | R::GroupInvitedJoin { group, .. } | R::GroupInvite { group, .. } => {
    ...
    pub fn sender(&self) -> Option<Uin> {
        ...
        match self {
            ...
            Event::Notice(n) => match n {
                ...
                N::Mute { operator, .. }
                | N::WholeMute { operator, .. }
                | N::GroupNameChange { operator, .. }
                | N::GroupDismiss { operator, .. }
                | N::ProfileLike { operator, .. } => Some(*operator),
                N::GroupNudge { sender, .. } => Some(*sender),
                N::FriendNudge { user, .. } | N::FriendFileUpload { user, .. } | N::FriendAdd { user } => Some(*user),
                N::GrayTip { user, .. } => *user,
                N::EssenceChange { .. } | N::PeerPinChange { .. } | N::BotOffline { .. } | N::Other { .. } => None,
            },
            Event::Request(r) => match r {
                R::Friend { initiator, .. }
                | R::GroupJoin { initiator, .. }
                | R::GroupInvitedJoin { initiator, .. }
                | R::GroupInvite { initiator, .. } => Some(*initiator),
            },
            Event::Meta(_) | Event::Raw(_) => None,
        }
    }
