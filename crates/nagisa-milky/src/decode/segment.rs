...
fn decode_segment(seg: &IncomingSegment, peer: Peer) -> Segment {
    match seg {
        ...
        IncomingSegment::Mention { user_id, name } => Segment::Mention { user: Uin(*user_id), name: name.clone() },
        ...
        IncomingSegment::Face { face_id, is_large } => {
            Segment::Face { id: face_id.clone(), large: *is_large, result_id: None, chain_count: None, sub_type: None }
        }
        IncomingSegment::Reply { message_seq, sender_id, time, segments, .. } => Segment::Reply {
        ...
        IncomingSegment::File { file_id, file_name, file_size, file_hash } => Segment::File {
            id: file_id.clone(),
        ...
        IncomingSegment::Forward { forward_id, title, preview, summary } => Segment::Forward(Forward::Ref {
            id: forward_id.clone(),
            title: title.clone(),
            preview: preview.clone(),
            summary: summary.clone(),
        }),
        IncomingSegment::MarketFace { emoji_package_id, emoji_id, key, summary, url } => Segment::MarketFace {
        ...
        IncomingSegment::LightApp { app_name, json_payload } => Segment::LightApp {
            app_name: (!app_name.is_empty()).then(|| app_name.clone()),
            payload: json_payload.clone(),
        },
        IncomingSegment::Xml { service_id, xml_payload } => {
            Segment::Xml { service_id: Some(*service_id), payload: xml_payload.clone() }
        }
        IncomingSegment::Unknown { kind, data } => {
            ...
            Segment::Raw { protocol: Protocol::Milky, kind: kind.clone(), data: data_map }
        }
    }
}
