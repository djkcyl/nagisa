...
```toml
nagisa = { version = "0.11.0", features = ["render"] }
```
...
| `---` | 分割线(`***` / `___` 同,3 个起步) |
| GFM 表格 | 分隔行 `\|:--\|:-:\|--:\|` 定列对齐 |
| `![图注](路径)` / `![图注](@名字)` | 块级图;`@名字` 从 `RenderOptions::images` 取字节;尾部可挂 `{width=50%, align=center, rounded=12, shadow, border=#ddd}` |
...
- **主题** `Theme`:亮 / 暗预设(`Theme::light()` / `Theme::dark()`)+ 全部配色 / 字族名 / 字号 / 行高 / 标题阶梯可改;代码上色盘 `CodePalette` 随主题(亮暗各一套,可自定)。
...
文档管线擅长「一大段排版」;游戏面板、战报、雷达属性卡这类**图形为主、文字为辅**的卡片,要精确摆放形状和短文本,文档流不趁手。`Canvas` 给一张像素级画布:形状走 tiny-skia 抗锯齿(与文档同后端,观感一致),文字复用整条版式管线(`Doc` → 排版 → 合成),CJK 整形 / 字体回退全部白拿。

```rust,ignore
use nagisa_render::{Canvas, Color, OutputFormat, RenderOptions};

let opts = RenderOptions::default();
let mut c = Canvas::new(520.0, 300.0, 2.0)?;                         // 逻辑宽高 + 超采样
c.rect(0.0, 0.0, 520.0, 300.0, 24.0, Color::rgb(0x16, 0x1b, 0x22)); // 圆角卡底
...
let png = c.encode(OutputFormat::Png)?;
```

坐标尺寸都是**逻辑像素**,内部统一乘 `scale` 换物理像素。图元:`rect` / `stroke_rect` / `v_gradient`(竖向渐变)/ `line` / `disc`(实心圆)/ `ring`(圆环)/ `arc`(圆弧)/ `polygon`(多边形,可填可描)/ `radar`(雷达图,样式见 `Radar`);文字:`text` / `text_mid`(水平居中)/ `text_doc`(整份 `Document` 落到坐标);合成 `blit`(贴一张 `RgbaImage`);出图 `encode`(→ 字节)或 `into_rgba`(→ RGBA 图,供再合成)。
